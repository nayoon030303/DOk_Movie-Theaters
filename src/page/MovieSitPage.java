...
public class MovieSitPage extends CategoryFrame {
   ...
   JButton[][] sit = new JButton[9][24];
   int[][] int_selectedSit = new int[9][24];//선택된 자석들 1
   JButton gray = new JButton(); //gray색 버튼
   
   String[] str_number = {"0","1", "2", "3", "4", "5", "6", "7", "8"};
   JLabel adult = new JLabel("성인");
   JComboBox comboboxAdult = new JComboBox(str_number);
   JLabel teen = new JLabel("청소년");
   JComboBox comboboxTeen = new JComboBox(str_number);
   JLabel kids = new JLabel("어린이");
   JComboBox comboboxKids = new JComboBox(str_number);
   JButton next = new JButton();
   JLabel[] row = new JLabel[24];
   JLabel[] column = new JLabel[9];
   JLabel[] selectRow = new JLabel[24];
   JLabel[] selectColumn = new JLabel[9];
   Vector<String> seatName = new Vector<String>();
               ...
               if(e.getSource() == sit[i][j] && (sit[i][j].getBackground() == gray.getBackground())) {
                  ...
                  if(num_adult == 0 && num_teen == 0 && num_kids == 0) {
                     ...
                     if(count <= selectCount ) {
                        ...
                        switch(i) 
                        {
                        case 0: select = "A" + (j + 1); break;
                        case 1: select = "B" + (j + 1); break;
                        case 2: select = "C" + (j + 1); break;
                        case 3: select = "D" + (j + 1); break;
                        case 4: select = "E" + (j + 1); break;
                        case 5: select = "F" + (j + 1); break;
                        case 6: select = "G" + (j + 1); break;
                        case 7: select = "H" + (j + 1); break;
                        case 8: select = "I" + (j + 1); break;
                        }
                        seatName.add(select);         
                        selectCount++;
                     }
