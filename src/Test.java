...
public class Test extends CategoryFrame implements Runnable {
	...
	public Test(User user, MovieArea pre_movieArea) {
		super("영화 좌석 선택");
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == next) {
				if (num_adult == 0 && num_teen == 0 && num_kids == 0) {
					...
					for (int i = 0; i < 9; i++) {
						for (int j = 0; j < 24; j++) {
							if (intdb_seatSit[i][j] == NO_SELECT && int_selectedSit[i][j] == NO_SELECT) {
								c++;
							}
						}
					}
					System.out.println("예약c:" + c + ", 선택한 좌석수:" + count);
					...
					if (c == count) {
						...
						// 예약된 좌석들
						// Thread t1
						// Thread t1 = new Thread(new ReservationCheckPage(user, num_adult, num_teen,
						// num_kids, seatName,ticket, movieArea));
						// t1.start();
					} else {
						JOptionPane.showMessageDialog(null, "이미 선택된 좌석입니다.");
						Test t1 = new Test(user, movieArea);
